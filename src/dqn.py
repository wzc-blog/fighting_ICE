...
class DQNPolicy():
    ...
    def __init__(self, params):
        ...
        # 是否使用duelingNet
        if params['is_dueling']:
            self.policy_network = DeulingNet(
                params['state_space_dim'],
                params['hidden_size'],
                params['action_space_dim'],
                params['compute_type'])
            self.target_network = DeulingNet(
                params['state_space_dim'],
                params['hidden_size'],
                params['action_space_dim'],
                params['compute_type'])
...
class DQNActor(Actor):
    ...
    def __init__(self, params):
        ...
        self.num_step = Tensor(params['num_step'], ms.int32)
        self.less = P.Less()
    ...
    def act(self, phase, params):
        ...
        if phase == 2:
            ...
            # total_rewards = Tensor([0], ms.float32)
            # total_is_terminal = Tensor([0], ms.int32)

            action = self.collect_policy(ts0, step_tensor)
            first_action = action
            new_state, reward, done = self._environment.step(action)
            total_rewards = reward
            total_is_terminal = done
            
            i = Tensor([1], ms.int32)
            while self.less(i, self.num_step):
                if done:
                    new_state = self._environment.reset()
                ts0 = self.expand_dims(new_state, 0)
                action = self.collect_policy(ts0, step_tensor)
                new_state, reward, done = self._environment.step(action)
                total_rewards += self.gamma ** i * reward
                total_is_terminal = done or total_is_terminal
                i += 1

            action = self.reshape(first_action, (1,))
            # my_reward = self.select(done, self.penalty, self.reward)
            my_reward = total_rewards
            done = total_is_terminal
...
class DQNLearner(Learner):
    ...
    def __init__(self, params=None):
        ...
        self.is_double_dqn = params['is_double_dqn']
    ...
    def learn(self, experience):
        ...
        if self.is_double_dqn:
            next_state_policy_values = self.policy_network(s1)
            a_max = P.Argmax(axis=1)(next_state_policy_values)
            a_max = self.expand_dims(a_max, 1)
            next_state_values = self.target_network(s1)
            next_state_values = self.gather(next_state_values, 1, a_max)
        ...
        y_true = r1 + self.gamma ** self.num_step * next_state_values

        # Modify last step reward
        one = self.ones_like(r1)
        y_true = self.select(r1 == -one, one, y_true) # 如果最后一步的奖励为-1，则将目标值修改为1
